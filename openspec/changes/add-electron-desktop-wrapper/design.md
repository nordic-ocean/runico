...
- **Electron with a thin main + preload, renderer = existing web app.** `main.js` creates the window and loads the web app; `preload.js` exposes a minimal, typed IPC surface via `contextBridge`. *Alternative:* Tauri — rejected for this user (Rust toolchain); Electron reuses the all-JS code with least friction.
