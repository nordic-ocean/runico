...
The system SHALL provide an Electron application that loads the existing `prototype/` Runico app as its renderer in a desktop window.

#### Scenario: Launching the desktop app
- **WHEN** the desktop app is started
- **THEN** a window opens showing the Runico interface from the existing prototype code
...
The system SHALL load the same renderer assets used by the web build, so the prototype is not duplicated or diverged for desktop.

#### Scenario: Single source of UI
- **WHEN** the renderer is updated in `prototype/`
...
- **THEN** the Electron app launches loading the local prototype
...
- **WHEN** the prototype is served as a static site as before
